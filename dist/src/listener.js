...
class Listener {
    ...
    call(message, middleware, didMatchCallback) {
        ...
        if (middleware == null) {
            middleware = new middleware_1.default(this.robot);
        }
        const match = this.matcher(message);
        if (match) {
            if (this.regex) {
                this.robot.logger.debug(`Message '${message}' matched regex /${util_1.inspect(this.regex)}/; listener.options = ${util_1.inspect(this.options)}`);
            }
...
class TextListener extends Listener {
    ...
    constructor(robot, regex, options, callback) {
        function matcher(message) {
            if (message instanceof message_1.TextMessage) {

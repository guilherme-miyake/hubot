...
const scoped_http_client_1 = __importDefault(require("scoped-http-client"));
// @ts-ignore
...
const listener_1 = __importStar(require("./listener"));
...
class Robot {
    ...
    constructor(adapterPath, adapter, httpd, name, alias = null) {
        if (name == null) {
            name = 'Hubot';
        }
        if (alias == null) {
        ...
        this.adapterPath = adapterPath ? adapterPath : path_1.default.join(__dirname, 'adapters');
        this.name = name;
        this.events = new events_1.EventEmitter();
        this.brain = new brain_1.default(this);
        this.alias = alias;
        this.adapter = null;
        this.datastore = null;
        ...
        this.listeners = [];
        this.middleware = {
            listener: new middleware_1.default(this),
            response: new middleware_1.default(this),
            receive: new middleware_1.default(this)
        };
        this.logger = new log_1.default(process.env.HUBOT_LOG_LEVEL || 'info');
    ...
    listen(matcher, options, callback) {
        this.listeners.push(new listener_1.default(this, matcher, options, callback));
    }
    ...
    hear(regex, options, callback) {
        ...
        this.listeners.push(new listener_1.TextListener(this, regex, options, callback));
    }
    ...
    enter(options, callback) {
        this.listen((msg) => msg instanceof message_1.EnterMessage, options, callback);
    }
    ...
    leave(options, callback) {
        this.listen((msg) => msg instanceof message_1.LeaveMessage, options, callback);
    }
    ...
    topic(options, callback) {
        this.listen((msg) => msg instanceof message_1.TopicMessage, options, callback);
    }
    ...
    receive(message, cb) {
        ...
        this.middleware.receive.execute({ response: new response_1.default(this, message, null) }, this.processListeners.bind(this), cb);
    }
    ...
    processListeners(context, done) {
        ...
        async_1.default.detectSeries(this.listeners, (listener, done) => {
            try {
        ...
        _ => {
            ...
            if (!(context.response.message instanceof message_1.CatchAllMessage) && !anyListenersExecuted) {
                this.logger.debug('No listeners executed; falling back to catch-all');
                this.receive(new message_1.CatchAllMessage(context.response.message), done);
            }
            else {
                if (done != null) {
    ...
    loadFile(filepath, filename) {
        ...
        const full = path_1.default.join(filepath, path_1.default.basename(filename, ext));
        ...
        try {
            ...
            if (typeof script === 'function') {
                script(this);
                this.parseHelp(path_1.default.join(filepath, filename));
            }
            else {
    ...
    load(path) {
        ...
        if (fs_1.existsSync(path)) {
            fs_1.readdirSync(path).sort().map(file => this.loadFile(path, file));
        }
    }
    ...
    setupExpress() {
        ...
        if (herokuUrl) {
            ...
            this.pingIntervalId = setInterval(() => {
                scoped_http_client_1.default.create(`${herokuUrl}hubot/ping`).post()((_err, res, body) => {
    ...
    parseHelp(path) {
        ...
        const body = fs_1.readFileSync(require.resolve(path), 'utf-8');
    ...
    http(url, options = {}) {
        const httpOptions = Object.assign(Object.assign({}, this.globalHttpOptions), options);
        return scoped_http_client_1.default.create(url, httpOptions).header('User-Agent', `Hubot/${this.version}`);
    }
}
module.exports = Robot;
function isCatchAllMessage(message) {
    return message instanceof message_1.CatchAllMessage;
}

...
import {CatchAllMessage, EnterMessage, LeaveMessage, TopicMessage} from "./message";
...
class Robot {
  ...
  listeners: any[];
  commands: any[];
  middleware: { listener: any; response: any; receive: any; };
  logger: any;
  pingIntervalId: any;
  globalHttpOptions: {};
  adapterName: any;
  ...
  constructor (adapterPath: string, adapter: string, httpd:boolean, name:string, alias:string| boolean = null) {
    if (name == null) {
      name = 'Hubot'
    }
    if (alias == null) {
    ...
    this.adapterPath = adapterPath? adapterPath : path.join(__dirname, 'adapters')
    ...
    this.on('error', (error: any, res:any) => {
      return this.invokeErrorHandlers(error, res)
    })
  ...
  //
  // Returns nothing.
  listen (matcher: any, options: object, callback: CallableFunction) {
  ...
  //
  // Returns nothing.
  hear (regex: RegExp, options: object, callback?: CallableFunction) {
  ...
  //
  // Returns nothing.
  respond (regex: RegExp, options: object, callback: CallableFunction) {
  ...
  //
  // Returns nothing.
  enter (options: object, callback: CallableFunction) {
    this.listen((msg:any) => msg instanceof EnterMessage, options, callback)
  }
  ...
  //
  // Returns nothing.
  leave (options: object, callback:CallableFunction) {
    this.listen((msg:any) => msg instanceof LeaveMessage, options, callback)
  }
  ...
  //
  // Returns nothing.
  topic (options: object, callback:CallableFunction) {
    this.listen((msg:any) => msg instanceof TopicMessage, options, callback)
  }
  ...
  //
  // Returns nothing.
  catchAll (options: object | CallableFunction, callback:CallableFunction) {
    // `options` is optional; need to isolate the real callback before
    // wrapping it with logic below
    if (callback == null && typeof options != 'object') {
      callback = options
      options = {}
    }

    this.listen(isCatchAllMessage, options, function listenCallback (msg:any) {
  ...
  //
  // Returns nothing.
  listenerMiddleware (middleware: CallableFunction) {
  ...
  //
  // Returns nothing.
  receiveMiddleware (middleware: Middleware) {
  ...
  receive (message: any, cb: CallableFunction) {
    // When everything is finished (down the middleware stack and back up),
    // pass control back to the robot
    this.middleware.receive.execute({ response: new Response(this, message, null) }, this.processListeners.bind(this), cb)
  }
  ...
  // Returns nothing.
  // Returns before executing callback
  processListeners (context: any, done:CallableFunction) {
    ...
    async.detectSeries(this.listeners, (listener: Listener, done:CallableFunction) => {
      try {
        listener.call(context.response.message, this.middleware.listener, function (listenerExecuted: any) {
  ...
  //
  // Returns nothing.
  loadFile (filepath: string, filename: any) {
  ...
  load (path: string) {
    ...
    if (existsSync(path)) {
      readdirSync(path).sort().map(file => this.loadFile(path, file))
    }
  }
  ...
  //
  // Returns nothing.
  loadHubotScripts (path:string, scripts: any) {
  ...
  //
  // Returns nothing.
  loadExternalScripts (packages: string[] | object) {
  ...
  parseHelp (path:string) {
    const scriptDocumentation:{[key: string] : string[]}
      = {
      commands: []
    }
    const body = readFileSync(require.resolve(path), 'utf-8')
  ...
  //
  // Returns nothing.
  messageRoom (room:any , ...strings:any) {
    const envelope = { room }
  ...
  http (url:string, options={}) {
    const httpOptions = {...this.globalHttpOptions,...options}
...
function isCatchAllMessage (message:any) {
  return message instanceof CatchAllMessage
}

function toHeaderCommentBlock (block:any, currentLine:any) {
  if (!block.isHeader) {
    return block
  }
...
function isCommentLine (line:any) {
  return /^(#|\/\/)/.test(line)
}
...
function extend (obj:{[key:string]:any}, ...sources:string[]) {

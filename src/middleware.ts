...
function withContext(self:any, context:Context, func: CallableFunction){
  return (...arg: any[]) => func(self, context,...arg)
}
...
export default class Middleware {
  ...
  // logic).
  executeSingleMiddleware (self:any, context:Context, stackItem: CallableFunction, middlewareFunc : CallableFunction, cb: CallableFunction) {
    // Match the async.reduce interface
    function nextFunc (newDoneFunc: CallableFunction | null) {
  ...
  execute (context: Context , next: (context: object, done: CallableFunction) => void, done: CallableFunction) {
    if (done == null) {
      done = function () {}
    }
    // Executed when the middleware stack is finished
    function allDone (_:any, finalDoneFunc:CallableFunction) {
      next(context, finalDoneFunc)
    }
    // Execute each piece of middleware, collecting the latest 'done' callback
    // at each step.
    process.nextTick(
      async.reduce.bind(null, this.stack, done, withContext(this, context, this.executeSingleMiddleware), allDone)
    )
  }
  ...
  //
  // Returns nothing.
  register (middleware: CallableFunction) {

...
let reconstructUserIfNecessary = function (user:any, robot: Robot) {
  if (!user) {
    return null
  }
...
export default class Brain extends EventEmitter {
  ...
  //
  // Returns nothing.
  resetSaveInterval (seconds: number) {
  ...
  usersForRawFuzzyName (fuzzyName:string) {
    ...
    return Object.keys(users).reduce((result: any[], key:string) => {
